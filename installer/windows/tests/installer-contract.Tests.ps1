...
Describe "PLwC Windows clean-machine prerequisite and UI contracts" {
    ...
    It "gates immutable r27 runtime image acquisition behind explicit interactive consent" {
        ...
        $managerSource | Should Match 'env=self\._child_environment\(\)'
        $managerSource | Should Match 'DOCKER_CONFIG'
        $managerSource | Should Match '\.anonymous-docker-config'
    ...
    It "uses one immutable r27 migration transaction and a hard postflight before success" {
        ...
        $maintenanceSource | Should Match 'parser\.add_argument\("--build-id",\s*required=True\)'
        $maintenanceSource | Should Match '"build_id":\s*args\.build_id'
        $maintenanceSource | Should Match '"plan_id":\s*_transaction_plan_id\(args\)'
        $maintenanceSource | Should Match '_running_report'
        $codeSection | Should Match '(?is)function\s+BuildInstallerMaintenanceArguments.*?--build-id'
        $codeSection | Should Match '(?is)function\s+RunInstallerMaintenance.*?DeleteFile\(GetInstallerMaintenanceReportPath\(ActionName\)\).*?PrimaryReportValid.*?ResultCode\s*:=\s*40'
